  ...
  <div class="pb-page">
    <div class="left-wrap" :class="{ 'is-collapsed': asideCollapsed, 'is-collapsing': asideCollapsing }">
          ...
          <template #header>
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-2">
                <span class="text-base font-medium">Velocity Proxy</span>
              </div>
              <div class="flex items-center gap-2">
      ...
      <div class="right-panel">
        ...
        <div v-else>
          <PluginConfigForm :plugin-key="'velocity_proxy'"
...
const isAllowed = (s:any) => normalizeServerType(s) === 'fabric'
...
const refreshInstalled = async () => {
  ...
  const tasks = servers.value.map(async (s:any) => {
    try {
      ...
      installedMap.value.set(s.id, hasModrinthSlug(mods, 'fabricproxy-lite'))
    } catch (e) { installedMap.value.set(s.id, false) }
  })
